...
import { createSession, destroySession, setActiveOrgCookie } from '@/server/session';
import { generateRandomToken } from '@/server/utils/tokens';
import { sendVerificationEmail, sendPasswordResetEmail } from '@/server/utils/email';
import { getCurrentUser } from '@/server/data/auth';
import PLANS from '@/server/config/plans';
import type { ActionState } from './types';

const generateSlug = (name: string): string =>
  name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') + '-' + Math.random().toString(36).slice(2, 6);
...
export async function loginAction(_prevState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    ...
    if (!user || !(await user.comparePassword(password))) return { error: 'Invalid email or password' };
...
export async function registerAction(_prevState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    ...
    const user = await User.create({ name, email, password, emailVerificationToken: verificationToken });
    const slug = generateSlug(orgName);
    const org = await Organization.create({ name: orgName, slug, owner: user._id, plan: 'free', limits: PLANS.free.limits });
    await Membership.create({ user: user._id, organization: org._id, role: 'owner', status: 'active' });
...
export async function forgotPasswordAction(_prevState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    ...
    // fall through — we intentionally don't reveal whether the email exists either way
  }
...
/** Sets the active-org cookie and lands on the dashboard. Called directly
 * from a native `<form action={selectOrgAction}>` on the org-select page —
...
export async function selectOrgAction(formData: FormData): Promise<void> {
  ...
  const membership = await Membership.findOne({ user: currentUser._id, organization: org._id, status: 'active' });
...
/** Called directly from the /invite/accept page's Server Component. */
export async function acceptInviteToken(token: string): Promise<{ ok: boolean }> {
  if (!token) return { ok: false };
  try {
    await connectDB();
    const membership = await Membership.findOne({ inviteToken: token, inviteExpires: { $gt: Date.now() }, status: 'invited' });
