...
export async function createCheckoutAction(formData: FormData): Promise<void> {
  ...
  try {
    ...
    console.error("createCheckoutAction error:", err);
    const message =
      err instanceof AppError ? err.message : "Could not start checkout";
...
export async function createPortalAction(): Promise<void> {
  ...
  try {
    ...
    if (!org.stripeCustomerId)
      throw new AppError("No billing account found", 400, "NOT_FOUND");
    ...
    const message =
      err instanceof AppError ? err.message : "Could not open billing portal";
    redirect(`/dashboard/billing?error=${encodeURIComponent(message)}`);
  }
