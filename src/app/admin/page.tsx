...
export default async function AdminOverviewPage() {
  const { stats, recentOrgs, recentUsers } = JSON.parse(
    JSON.stringify(await getAdminStats()),
  );
  ...
  return (
    <>
      ...
      <Grid
        ...
        mb="8"
      >
        {Object.entries(stats.plans as Record<string, number>).map(
          ([plan, count]) => (
            <Box
              key={plan}
              bg="bg.surface"
      ...
      <Grid templateColumns={{ base: "1fr", lg: "1fr 1fr" }} gap="6">
        <Box>
          <Flex justify="space-between" align="center" mb="4">
            ...
            <ChakraLink href="/admin/orgs" fontSize="xs" color="violet.400">
              View all →
            </ChakraLink>
          </Flex>
          {!recentOrgs.length ? (
        ...
        <Box>
          <Flex justify="space-between" align="center" mb="4">
            ...
            <ChakraLink href="/admin/users" fontSize="xs" color="violet.400">
              View all →
            </ChakraLink>
          </Flex>
          {!recentUsers.length ? (
