...
export default async function AdminOrgsPage({
  ...
  const plan =
    planParam === "pro" || planParam === "enterprise" || planParam === "free"
      ? planParam
      : "";

  const { organizations, pagination } = await getAllOrgs({
    page,
    search,
    plan,
  });
        ...
        <Flex direction="column" gap="2">
            ...
            >
              <Box>
                ...
                <Text fontSize="xs" color="text.muted">
                  {org.owner?.email} · {org.memberCount} members ·{" "}
          ...
          {pagination.pages > 1 && (
            <Flex justify="space-between" align="center" mt="2">
              <ChakraLink
                href={`/admin/orgs?page=${Math.max(1, page - 1)}${search ? `&search=${search}` : ""}`}
                fontSize="sm"
              ...
              <ChakraLink
                href={`/admin/orgs?page=${Math.min(pagination.pages, page + 1)}${search ? `&search=${search}` : ""}`}
                fontSize="sm"
                color="text.secondary"
              >
                Next →
