  ...
  return (
    <>
      ...
      <form method="get" action="/admin/users">
        <Input name="search" defaultValue={search} placeholder="Search users..." maxW="320px" mb="5" {...inputStyle} />
      </form>
          ...
          {list.map((u: any) => (
            <Flex key={u._id} justify="space-between" align="center" bg="bg.surface" border="1px solid" borderColor="border.subtle" borderRadius="xl" p="4">
              ...
              <Flex align="center" gap="3">
                ...
                <Text fontSize="xs" color="text.muted">{new Date(u.createdAt).toLocaleDateString()}</Text>
              </Flex>
            </Flex>
          ))}
          {pagination.pages > 1 && (
            <Flex justify="space-between" align="center" mt="2">
              <ChakraLink href={`/admin/users?page=${Math.max(1, page - 1)}${search ? `&search=${search}` : ''}`} fontSize="sm" color="text.secondary">← Prev</ChakraLink>
              <Text fontSize="xs" color="text.muted">{page} / {pagination.pages}</Text>
              <ChakraLink href={`/admin/users?page=${Math.min(pagination.pages, page + 1)}${search ? `&search=${search}` : ''}`} fontSize="sm" color="text.secondary">Next →</ChakraLink>
            </Flex>
          )}
        </Flex>
      )}
